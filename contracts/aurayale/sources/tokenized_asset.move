module aurayale::tokenized_asset {
    ...
    public struct TOKENIZED_ASSET has drop {}
    ...
    /// The AssetMetadata public struct defines the metadata representing the entire asset.
    ...
    /// TokenizedAsset(TA) public struct represents a tokenized asset of type T.
    ...
    public fun metadata<T>(t_asset: &TokenizedAsset<T>): VecMap<String, String> {
        t_asset.metadata
    }
    ...
    fun init(otw: TOKENIZED_ASSET, ctx: &mut TxContext) {
        transfer::transfer(
            PlatformCap {
                id: object::new(ctx),
            },
    ...
    public fun update_display<T>(publisher: &mut Publisher, ctx: &mut TxContext) {
        let keys = vector[b"image_url".to_string()];

        let values = vector[b"{image_url}".to_string()];
        let mut display = display::new_with_fields<TokenizedAsset<T>>(
            publisher,
            keys,
            values,
            ctx,
    ...
    public fun mint<T>(
        ...
        TokenizedAsset {
            ...
            balance,
            metadata,
            image_url: image_url.extract_or!(ascii::string(b"")),
        }
    }
